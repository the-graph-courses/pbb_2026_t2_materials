...
Before running this on the whole dataset, let's test it on a single abstract to see what the output looks like:

```{python}
test_abstract = "We conducted a randomized controlled trial to evaluate a new insulin regimen for patients with type 2 diabetes, measuring its effect on long-term blood glucose control."

classify_disease_focus(test_abstract)
```

Now that we've confirmed the function works, let's vectorize it so we can apply it to an entire column, then run it on every abstract in the `papers` DataFrame:
...
Load the data into a pandas dataframe from the `data` folder:
...
We have written the prompt for you. First, run the chunk below to define the function:
...
Now test the function on a single short job description to see what it returns:

```{python}
test_description = "We are seeking a data analyst with at least 3 years of experience working with Python and SQL in a healthcare setting."

yrs_exp(test_description)
```

Now vectorize the function and apply it on the `job_description` column of the dataframe. Create a column called `years_required` in the dataframe to store the results.
...
First, define the function. We've given you the function name and signature to get you started — you write the prompt:

```{python}
def classify_lang(description):
    prompt = f"""
    # Your prompt here
    {description}
    """
...
Now test your function on a single short job description to see what it returns:

```{python}
test_description = "Looking for a statistician comfortable building dashboards in R and writing reproducible analysis pipelines."

# Run your function on test_description here
classify_lang(test_description)
```

Now vectorize the function (we've written this for you) and apply it on the `job_description` column. Store the results in a new column called `lang_req`:

```{python}
# Vectorize the function
classify_lang_vec = np.vectorize(classify_lang)
...
# Optional Ungraded Challenge: Most common technical skills overall
