...
## 2.1. Table of Costs by Education Level

Let's compare the cost of treatment across different education levels using a summary table. Use pandas methods to group by `education level` and create a summary table with two variables:
...
Reminder: in the prework you used this pattern to summarize grouped data. This time, use `count` and `mean` as the summary functions:
...
## 2.2. Visualize Costs by Education Level

Using `plotly.express` and the costs summary table you created earlier, create a bar plot of mean costs by education level.
